namespace System
{
	...
	{
		public static int ByteLength_ (Array array)
		{
		...
		public static void BlockCopy_ (Array src, int srcOffset, Array dst, int dstOffset, int count)
		{
			if (src == null)
			...
			// bool 
            int res = InternalBlockCopy (src, srcOffset, dst, dstOffset, count);
            // if (!res) {
            if (res > 0)
            {
		...
		internal static unsafe void Memcpy_(byte *dest, byte *src, int size)
        {
